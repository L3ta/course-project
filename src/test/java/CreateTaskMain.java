...
public class CreateTaskMain {
    ...
    @Test
    @Description("Тест на створення задачі")
    ...
    private void executeFlowInBrowser(String browser) {
        String ownerId = createUser(); // Create user

        if (ownerId != null) {
            String projectId = createProject(ownerId); // Create project

            if (projectId != null) {
                CreateTask taskManager = new CreateTask();
